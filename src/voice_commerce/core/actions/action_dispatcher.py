...
def _infer_sort_action(tool_name: str, tool_args: dict[str, Any]) -> BrowserAction | None:
    ...
    if any(
        token in normalized
        for token in ("cheapest", "lowest price", "low price", "price low", "budget")
    ):
...
class ActionDispatcher:
    ...
    def _on_search_products(
        ...
        # First highlight at 2.5s - gives time for Gemini to start speaking about first result
        # Stagger by 3.5s between products - slow enough to see each one clearly
        base_delay_ms = 2500
        STAGGER_MS = 3500
        FADE_MS = 18000  # Each highlight stays visible for 18s
