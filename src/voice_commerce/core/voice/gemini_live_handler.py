...
class GeminiLiveHandler:
    ...
    def __init__(
        self, transcript: list[dict[str, str]], resumption_handle: str | None = None
    ) -> None:
        """Prepare one Gemini Live session wrapper for a browser conversation."""
        ...
        log.info(
            ...
            summary_warmup_fallback=self._category_summary_text.startswith(
                "Category intelligence is warming up"
            ),
        )
    ...
    def _handle_send_exception(
        self, action: str, exc: Exception, *, tool_name: str | None = None
    ) -> bool:
        """
    ...
    async def inject_live_context(self, page: dict, products: list[dict]) -> None:
        ...
        for index, p in enumerate(products, start=1):
            product_text += (
                f"{index}. ID: {p.get('id')} | Name: {p.get('name')} | Price: {p.get('price')}\n"
            )
        ...
        try:
            await self._session.send_client_content(
                turns=types.Content(role="user", parts=[types.Part(text=context_msg)]),
                turn_complete=True,
            )
        except Exception as exc:
        ...
        try:
            ...
            while True:
                ...
                async for response in self._session.receive():
                    ...
                    if server_content:
                        ...
                        if model_turn and model_turn.parts:
                            for part in model_turn.parts:
                                ...
                                if getattr(part, "text", None):
                                    log.debug(
                                        "Received gemini_text_chunk", length=len(part.text or "")
                                    )
                        ...
                        if output_trans and getattr(output_trans, "text", None):
                            log.debug(
                                "gemini_output_transcript", text_length=len(output_trans.text)
                            )
                ...
                if not saw_message_in_this_receive_call:
                    log.debug(
                        "gemini_receive_no_message",
                        message="No messages received in this call to session.receive(). This may indicate a silent disconnection or a network issue.",
                    )
                    yield {
                        "type": "session_closed",
                        "reason": "receive() finished AS silent_disconnect",
                    }
