"""Shared audio format constants for the browser and Gemini Live session."""
...
GEMINI_OUTPUT_BYTES_PER_SECOND: int = (
    GEMINI_OUTPUT_SAMPLE_RATE * GEMINI_OUTPUT_BYTES_PER_SAMPLE * GEMINI_OUTPUT_CHANNELS
...
def is_valid_audio_chunk(data: bytes) -> bool:
    """Return True when the PCM chunk looks aligned to 16-bit samples."""
    if not data:
        return False
...
def chunk_duration_ms(chunk: bytes) -> float:
    """Estimate playback duration for one PCM chunk."""
    if not chunk:
        return 0.0
...
def get_browser_audio_config() -> dict:
    """Return the output audio format the browser should play back."""
    return {
...
def get_mic_audio_config() -> dict:
    """Return the input audio format the browser should capture from the mic."""
    return {
